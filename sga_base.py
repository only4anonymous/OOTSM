...
class SGABase:
    ...
    def _init_config(self):
        ...
        if self._conf.distributed:
            ...
            self._device = torch.device("cuda", self._conf.local_rank)
        else:
    ...
    def _init_optimizer(self):
        ...
        if self._conf.optimizer == const.ADAMW:
            self._optimizer = AdamW(trainable_params, lr=self._conf.lr)
        elif self._conf.optimizer == const.ADAM:
            self._optimizer = optim.Adam(trainable_params, lr=self._conf.lr)
        elif self._conf.optimizer == const.SGD:
            self._optimizer = optim.SGD(trainable_params, lr=self._conf.lr, momentum=0.9, weight_decay=0.01)
        else:
