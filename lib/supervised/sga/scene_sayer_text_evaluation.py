...
class SceneSayerODE(nn.Module):
    ...
    def forward_single_entry(self, context_fraction, entry):
        ...
        # # print(f"Stage 0 生成的文本：{stage0_generated_text}")


        # # future_objects_by_frame = self.parse_stage0_output(stage0_generated_text, future_frames)
        # future_objects_by_frame = self.stage0_anticipator.parse_generated_objects(stage0_generated_text, time=False)
        ...
        for obj_cls in all_objects:
            ...
            if not obs_obj_segments:
                continue
            observed_text = self._build_text_from_segments_for_object(obs_obj_segments, obj_cls, observed=True, max_tokens=1500).split("\n")
            # 如果有obj_to_future_frames就num_future_obj = len(obj_to_future_frames[obj_cls])，否则就
        ...
        while attempt < max_attempts and pending_indices:
            ...
            for idx_in_batch, gen_text in enumerate(batch_generated):
                ...
                lines_parsed, line_id = self.extract_future_segments(lines_raw, obj_to_future_frames[obj_list[i_obj]])
    ...
    def extract_future_segments(self, lines, future_num):
        ...
        for i, frame_id in enumerate(raw_frame_ids):
            if frame_id in future_num:  # 保留无法识别帧号的行
