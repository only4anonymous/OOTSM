...
class SceneSayerODE(nn.Module):
    ...
    def forward_single_entry(self, context_fraction, entry):
        """
        推理主入口（修改后的版本）：  
        1. 利用 DS-GDETR 处理当前帧，获取 im_idx、pair_idx、gt_annotation、frame_idx 等信息；  
        2. 根据 context_fraction 计算观测段结束帧 end，并确定未来帧数 num_future；  
        3. 利用 DS-GDETR 输出构造 frames_annotation 及 frames_ranges（用于后续构造 pair_idx 等）；  
        4. 从 observed anno 中提取各对象的段信息，并从最后一帧中获得目标对象集合 all_objects；  
        5. 对每个目标对象：构造 prompt、调用 llm_anticipator.generate_text 得到生成文本，再解析出未来帧的文本行，
        并一次性调用分类头对这些文本行进行预测，得到 [num_future, 26] 的预测分布；  
        6. 将所有对象的预测结果存入 distribution_dict，再调用 generate_relationship_distributions 得到
        attn_mat, spat_mat, cont_mat；  
        7. 调用 build_pair_idx_im_idx_and_boxes 构造新的 pair_idx、im_idx_tensor、boxes；  
        8. 将预测结果打包到 pred 字典中返回，同时返回 end+1。
        """
        device = entry["im_idx"].device
        ...
        im_idx = entry["im_idx"]         # tensor, shape [N]
        pair_idx = entry["pair_idx"]     # tensor, shape [N, 2]
        gt_annotation = entry["gt_annotation"]  # list of frames
        ...
        # 2) 根据 context_fraction 计算观测端结束帧 end 及未来帧数
        ...
        # 3) 构造 frames_ranges（基于 entry["im_idx"] 和 pair_idx）
        # 以下逻辑与训练代码中保持一致
        bool_diff = (im_idx[:-1] != im_idx[1:])
        indices     = bool_diff.nonzero().view(-1) + 1
        frames_ranges = torch.cat([torch.tensor([0], device=device), indices, torch.tensor([im_idx.size(0)], device=device)]).long()
        ...
        if end >= num_frames - 1 or frames_ranges[end] == frames_ranges[end + 1]:
            return num_frames, {}
        # 4) 获取 scene 信息：构造 frames_annotation 和按对象分组
        ...
        # 这里采用训练代码的辅助函数来合并帧：用于 inference 的版本
        ...
        # 选取最后一帧出现的对象
        last_frame_data = observed_anno[-1]
        last_objects = set()
        if len(last_frame_data) > 1:
            for obj in last_frame_data[1:]:
                if 'class' in obj:
                    cidx = obj['class']
                    if 0 <= cidx < len(self.obj_classes):
                        last_objects.add(self.obj_classes[cidx])
        all_objects = last_objects

        # 5) 对每个目标对象构造 prompt、生成文本、解析文本、并一次性分类得到预测分布
        dist_for_objects = {}
        for obj_cls in all_objects:
            segs_for_obj = obs_by_obj.get(obj_cls, [])
            if not segs_for_obj:
                continue
            # (a) 构造观测端文本（按行拆分）
            observed_text = self._build_text_from_segments_for_object(segs_for_obj, obj_cls, observed=True).split("\n")
            # (b) 用大模型一次性生成 window 帧描述
            dist_mat = self.get_relationship_distribution_for_object(observed_text, obj_cls, num_future)
            dist_for_objects[obj_cls] = dist_mat  # [num_future, 26]

        # 6) 调用 generate_relationship_distributions 得到各关系分布矩阵
        attn_mat, spat_mat, cont_mat = self.generate_relationship_distributions(
            distribution_dict=dist_for_objects,
            end=end,
            num_future=num_future
        )

        # 7) 调用 build_pair_idx_im_idx_and_boxes 构造新的 pair_idx, im_idx_tensor, boxes
        num_objs = int(frames_ranges[end+1] - frames_ranges[end])
        ...
        if attn_mat.shape[0] == 0:
            ...
            pred["pair_idx"] = new_pair_idx.to(device)
            pred["im_idx"] = im_idx_tensor.to(device)
            pred["boxes"] = boxes.to(device)

        # 9) 如果需要复制 scores/labels，则根据 DS-GDETR 的输出进行复制
    ...
    def get_relationship_distribution_for_object(self, observed_text_list, obj_cls, num_future):
        """
        针对单个对象，利用大模型生成未来帧文本，再从生成文本中直接查找特殊 token（<frame>, <obj>, <att>, <spat>, <cont>）
        并利用对应的隐藏状态经过分类头得到预测分布。  
        输入：
        observed_text_list: List[str]，观测端文本行（例如由 _build_text_from_segments_for_object 得到并 split("\n")）
        obj_cls: 当前对象类别（字符串，可用于 prompt 显示）
        num_future: 需要预测的未来帧数
        返回：
        dist_mat: Tensor, shape [num_future, NUM_REL_CLASSES]，每行表示对应帧的关系预测分布
        """
        device = self.llm_anticipator.device

        # (a) 构造 prompt：要求生成未来 num_future 帧描述
        prompt = self.llm_anticipator.build_prompt_for_scene_graph(
            observed_segments=observed_text_list,
            object_class=obj_cls,
            relationship_categories=self.relationship_categories,
            num_future_frames=num_future
        )
        generated_text = self.llm_anticipator.generate_text(prompt, max_new_tokens=512)
        generated_new_text = generated_text.replace(prompt, "")
        # (b) 调用生成函数
        enc = self.llm_anticipator.tokenizer(generated_new_text, add_special_tokens=True, return_tensors="pt").to(device)
        
        # (c) 输入到 joint_model 中获得输出
        with torch.no_grad():
        ...
        hidden_states = outputs.hidden_states[-1][0]  # [seq_len, hidden_size]，假设 batch_size=1
        input_ids_generated = enc["input_ids"][0]       # [seq_len]

        # (d) 获取特殊 token 的 token id
        frame_token_id = self.llm_anticipator.tokenizer.convert_tokens_to_ids("<frame>")
        obj_token_id   = self.llm_anticipator.tokenizer.convert_tokens_to_ids("<obj>")
        att_token_id   = self.llm_anticipator.tokenizer.convert_tokens_to_ids("<att>")
        spat_token_id  = self.llm_anticipator.tokenizer.convert_tokens_to_ids("<spat>")
        cont_token_id  = self.llm_anticipator.tokenizer.convert_tokens_to_ids("<cont>")

        # (e) 在生成文本中查找所有 <frame> 的位置
        frame_positions = (input_ids_generated == frame_token_id).nonzero(as_tuple=False).squeeze(-1)
        dist_list = []
        # 遍历每个检测到的帧区域
        for i in range(len(frame_positions)):
            start_idx = frame_positions[i].item()
            end_idx = input_ids_generated.size(0) if i == len(frame_positions) - 1 else frame_positions[i+1].item()
            region = input_ids_generated[start_idx:end_idx]
            # 辅助函数：在区域内查找某 token 的第一次出现
            def find_first(token_id):
                pos_candidates = (region == token_id).nonzero(as_tuple=False)
                if pos_candidates.numel() > 0:
                    return start_idx + pos_candidates[0].item()
                else:
                    return None
            pos_obj  = find_first(obj_token_id)
            pos_att  = find_first(att_token_id)
            pos_spat = find_first(spat_token_id)
            pos_cont = find_first(cont_token_id)
            if pos_obj is None or pos_att is None or pos_spat is None or pos_cont is None:
                continue  # 跳过当前帧
            hs_obj  = hidden_states[pos_obj, :]
            hs_att  = hidden_states[pos_att, :]
            hs_spat = hidden_states[pos_spat, :]
            hs_cont = hidden_states[pos_cont, :]
            # 这里简单采用四个 token 的隐藏状态均值作为该帧的表示
            frame_rep = torch.stack([hs_obj, hs_att, hs_spat, hs_cont], dim=0).mean(dim=0)
            logits = self.llm_anticipator.joint_model.classifier(frame_rep)
            probs = torch.sigmoid(logits)
            dist_list.append(probs)
        
        # (f) 如果检测到的帧数不足 num_future，则复制最后一帧的结果补齐；如果过多则只取前 num_future
        if len(dist_list) == 0:
            dist_mat = torch.zeros(num_future, NUM_REL_CLASSES, device=device)
        else:
            if len(dist_list) < num_future:
                last = dist_list[-1]
                for _ in range(num_future - len(dist_list)):
                    dist_list.append(last)
            elif len(dist_list) > num_future:
                dist_list = dist_list[:num_future]
            dist_mat = torch.stack(dist_list, dim=0)  # [num_future, NUM_REL_CLASSES]
        return dist_mat
    ...
    def generate_relationship_distributions(
        ...
        for i_frame in range(num_future):
            ...
            for j_obj, obj_cls in enumerate(obj_list):
                dist_mat = distribution_dict[obj_cls]   # [K, 26]
                frames    = dist_mat.shape[0] # [K]
    ...
    def _build_text_from_segments_for_object(self, obj_segments, obj_cls, observed=True):
        ...
        for i, seg in enumerate(obj_segments):
            ...
            line = self._construct_segment_text(start_time, seg['end_time'], seg, obj_cls, include_time=False)
            lines.append(line)
        return "\n".join(lines)

    def _construct_segment_text(self, start_time, end_time, seg, obj_cls, segment_type="observed", include_time=True):
        frame_marker = f"<frame>{start_time}-{end_time}</frame>"
        if segment_type == "observed":
            obj_token = f"<observed_obj>{obj_cls}</observed_obj>"
        else:
            obj_token = f"<future_obj>{obj_cls}</future_obj>"
        attn_content = ",".join([self.rel_classes[id_] for id_ in seg["attn_ids"]]) if seg["attn_ids"] else "None"
        spat_content = ",".join([self.rel_classes[id_] for id_ in seg["spat_ids"]]) if seg["spat_ids"] else "None"
        cont_content = ",".join([self.rel_classes[id_] for id_ in seg["cont_ids"]]) if seg["cont_ids"] else "None"
        att_token = f"<att>{attn_content}</att>"
        spat_token = f"<spat>{spat_content}</spat>"
        cont_token = f"<cont>{cont_content}</cont>"
        if include_time:
            return f"{frame_marker} {obj_token} {att_token} {spat_token} {cont_token}."
        else:
            return f"{obj_token} {att_token} {spat_token} {cont_token}."
...
class SceneGraphAnticipator:
    ...
    def __init__(self, model_path, lora_path, classifier_path, device='cuda', FP16=False):
        ...
        # 添加特殊标记
        special_tokens = {"additional_special_tokens": ["<obj>", "</obj>",
                                                        "<att>", "</att>",
                                                        "<spat>", "</spat>",
                                                        "<cont>", "</cont>",
    ...
    def build_prompt_for_scene_graph(self, observed_segments, object_class, relationship_categories, num_future_frames=1):
        header = (
            ...
            "Format instructions:\n"
            "  - Observed frames are enclosed with <observed_obj> ... </observed_obj>.\n"
            "  - Future frames are enclosed with <future_obj> ... </future_obj>.\n"
            "  - Each frame starts with a <frame> token (e.g., <frame>start-end</frame>),\n"
            "    followed by special tokens: <obj>, <att>, <spat>, <cont> (each with a matching closing tag).\n"
            "  - Missing any special token will incur a penalty.\n"
        )
        instruction_0 = (
            f"\nPlease generate the future segment for object [{object_class}] in the exact structured format as specified above.\n"
    ...
    def generate_text(self, prompts, max_new_tokens=256, temperature=0.9, top_p=0.7):
        ...
        # decode
        decoded = [self.tokenizer.decode(outputs[i], skip_special_tokens=False) for i in range(outputs.size(0))]
