...
from llama_SGA_new.train_llm_scene_graph_predictor_plain import ScriptFineTuner
...
class STTran(nn.Module):
    ...
    def print_indices(self, entry):
        ...
        return subj_class, obj_class, attn_rel_indices, spaitial_rel_indices, contacting_rel_indices, frame_idx
...
class SceneSayerODE(nn.Module):
    ...
    def __init__(self, mode, attention_class_num=None, spatial_class_num=None, contact_class_num=None, obj_classes=None,
        ...
        self.ctr = 0
        self.script_finetuner = ScriptFineTuner(
            mode='inference',              # 推理模式
            model_path=llama_path,         # 你的LLM模型路径
            local_rank=0,                  # 若只单GPU可写死0
            world_size=1,                  # 同理
            enable_classifier=True,        # 如果想用分类器
            ag_object_classes=self.obj_classes,  # 传入你的类别名列表
            ckpt_path=lora_path      # 你微调好的checkpoint路径
        )
    ...
    def forward_single_entry(self, context_fraction, entry):
        ...
        subj_class, obj_class, attn_rel_indices, sp_rel_indices, cont_rel_indices, frame_idx = self.dsgdetr.print_indices(entry)
        observed_anno = self.build_frames_annotation(
            im_idx, obj_class, attn_rel_indices, sp_rel_indices, cont_rel_indices, frame_idx
        )[: end+1]
        ...
        while retry_count < max_retries:
            im_idx_list, labels_list, attn_list, spat_list, cont_list = \
            self.script_finetuner.predict_future_frames_single(
                frames=observed_anno,      # 观测帧
                frame_indices=frame_idx[end+1:],   # 观测帧编号
                max_new_length=256 + retry_count*100,
                do_sample=True,
                temperature=0.7
            )
            generated_end = len(im_idx_list)
            for i, im_idx in enumerate(im_idx_list):
                if im_idx == 0:
                    generated_end = i
            im_idx_list = im_idx_list[: generated_end]
            labels_list = labels_list[: generated_end]

            # 检查结果是否为空
            if len(im_idx_list) == future_num_frames and all(idx > 0 for idx in im_idx_list):
                break
            
            
            retry_count += 1
            if retry_count == max_retries:
                print(f"Warning: Empty results after {max_retries} attempts")
                return num_frames, {}
    ...
    def build_pred_from_future_frames(self, num_future_frames, im_idx_list, labels_list,
        ...
        """
        # 如果生成的帧数不够，复制最后一帧的结果（如果存在）补全
        current_frames = len(im_idx_list)
        if current_frames < num_future_frames:
            if current_frames > 0:
                last_im = im_idx_list[-1]
                last_labels = labels_list[-1]
                last_attn = attn_list[-1].unsqueeze(0)  # 增加一个维度用于拼接
                last_spat = spat_list[-1].unsqueeze(0)
                last_cont = cont_list[-1].unsqueeze(0)
                # 计算需要补充的帧数
                frames_to_add = num_future_frames - current_frames
                
                # 使用torch.cat直接拼接tensor
                attn_list = torch.cat([attn_list, last_attn.repeat(frames_to_add, 1)], dim=0)
                spat_list = torch.cat([spat_list, last_spat.repeat(frames_to_add, 1)], dim=0)
                cont_list = torch.cat([cont_list, last_cont.repeat(frames_to_add, 1)], dim=0)
                
                # 对于非tensor的列表，继续使用append
                for _ in range(current_frames, num_future_frames):
                    im_idx_list.append(last_im)
        ...
        im_idx_list = im_idx_list[:num_future_frames]
        ...
        if total_objects > 0:
            # 这里如果拼接后的行数大于 total_objects，则取前 total_objects 行
            attn_concat = attn_list[:total_objects]
            spat_concat = spat_list[:total_objects]
            cont_concat = cont_list[:total_objects]
            pred["attention_distribution"] = attn_concat.to(device)  # shape: [total_objects, 3]
            pred["spatial_distribution"] = spat_concat.to(device)     # shape: [total_objects, 6]
            pred["contacting_distribution"] = cont_concat.to(device)    # shape: [total_objects, 17]
        ...
        if total_objects != len(pred["im_idx"]):
            breakpoint()
        return pred
    ...
    def build_frames_annotation(
        ...
        for i in range(len(im_idx)):
            if i==0:
                frame_id = frame_idx[j]
            elif im_idx[i] == im_idx[i-1]:
                frame_id = frame_idx[j]
            else:
                j += 1
                frame_id = frame_idx[j]
