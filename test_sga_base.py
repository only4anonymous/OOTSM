...
from lib.supervised.evaluation_recall_parse import BasicSceneGraphEvaluator
...
class TestSGABase(SGABase):
    ...
    def __init__(self, conf):
        ...
        if self._conf.fraction is not None:
            self._context_fractions = [self._conf.fraction]
        else:
            self._context_fractions = [0.9]
    ...
    def _write_evaluation_statistics(self, is_future_frame=True):
        ...
        results_dir = os.path.join(os.getcwd(), self._conf.results_path)
    ...
    def compute_scene_sayer_context_score(self, result, gt_annotation, context_fraction, windowsize):
        pred = result["pred"]
        ind = result["ind"]
        window_end = ind + windowsize if ind + windowsize < len(gt_annotation) else len(gt_annotation)
        # self._evaluate_predictions(context_fraction, gt_annotation[ind:window_end], pred)
        ...
        with torch.no_grad():
            ...
            for i, context_fraction in enumerate(self._context_fractions):
                ...
                for num_video_id in tqdm(range(len(self._dataloader_test)), desc=f"Testing Progress (Context Fraction: {context_fraction})",
                    ...
                    data, script, video_id = next(test_iter) #len(data) = 5 data[0].shape = torch.Size([9, 3, 1067, 600])
                    ...
                    # ----------------- Process the video (Method Specific) --------------------- 
                    result = self.process_test_video_context(entry, frame_size, gt_annotation, context_fraction, script_embeddings)
                    # ---------------------------------------------------------------------------
